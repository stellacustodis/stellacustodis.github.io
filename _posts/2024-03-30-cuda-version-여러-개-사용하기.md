...
**what is cuda?**
모두 알다시피 nvidia는 gpu를 만드는 회사이다. 얘네가 무서운 이유는 하드웨어만 설계하는 것이 아니라 딥러닝에 사용할 수 있는 소프트웨어도 추가로 제공을 해준다. 그게 cuda와 cudnn이라고 생각을 하면 된다.
...
10. 매번 ~./bashrc를 변경해야한다고 인터넷에 나와있는 경우도 있는데, 소프트링크만 cuda로 재설정하는 경우라면 건들지 않아도 된다.
