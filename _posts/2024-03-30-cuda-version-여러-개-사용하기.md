...
모두 알다시피 nvidia는 gpu를 만드는 회사이다. nvidia가 무서운 이유는 하드웨어만 설계하는 것이 아니라 딥러닝에 사용할 수 있는 소프트웨어도 추가로 제공을 해준다. CUDA는 NVIDIA GPU를 위한 병렬 컴퓨팅 플랫폼과 toolkit이고, cuDNN은 CUDA 위에서 딥러닝 연산을 최적화한 라이브러리라고 생각하면 된다.
...
3. 하단의 캡쳐는 cuda toolkit 11.8을 다운로드하는 nvidia 홈페이지이다. 사용하는 ubuntu 버전에 따라 설치하는 cuda의 버전이 제한된다. CUDA 릴리스마다 지원하는 배포판과 컴파일러가 다르므로 설치하려는 버전의 호환성 표를 확인해야 한다.
...
5. 알려주는 명령어를 terminal창에 고대로 입력을 하면 된다. runfile은 다음처럼 `sh`로 실행할 수 있다.
...
6. 그리고 설치를 해주면 되는데, 이미 설치된 NVIDIA driver가 해당 CUDA Toolkit의 최소 요구 버전을 만족하는지 먼저 확인한다. 호환되는 driver가 있다면 runfile 설치 화면에서 driver 항목을 해제해 중복 설치하지 않는다.
...
9. 현재 컴퓨터 전체에 적용된 cuda 소프트링크를 변경하려면 다음의 예시를 참고하면 된다. 이 방식은 모든 사용자에게 영향을 주므로 단독으로 관리하는 컴퓨터에서만 사용한다.
