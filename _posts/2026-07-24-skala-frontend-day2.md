...
폼 컨트롤은 전부 `<input>` 태그이므로, `type`별로 다르게 꾸미려면 속성 선택자가 사실상 필수다.
...
이 태그가 없으면 모바일 브라우저가 데스크톱 너비(보통 980px)를 가정하고 페이지 전체를 축소해서 보여준다. **미디어 쿼리를 아무리 잘 짜도 이 태그가 없으면 동작하지 않는다.**
...
**Mobile First가 권장된다.** 작은 화면에서 시작하면 무엇이 정말 필요한 콘텐츠인지 먼저 결정하게 되고, 모바일 기기가 불필요한 데스크톱용 스타일을 내려받지 않아 로딩에도 유리하다.
...
| 특징 | 설명 |
|---|---|
| **Interpreter Language** | 별도 컴파일 없이 브라우저가 코드를 읽으며 즉시 실행 |
| **Dynamic Typing** | 변수 선언 시 타입을 지정하지 않고, 값이 할당될 때 타입이 결정됨 |
| **Single Thread** | 한 번에 하나의 작업만 처리. 대신 비동기로 화면 멈춤을 방지 |

싱글 스레드라는 점이 이 글 후반부의 비동기 처리 전체를 지배한다. **자바스크립트는 한 번에 하나만 할 수 있기 때문에**, 서버 응답을 기다리는 동안 멈추면 페이지 전체가 멈춘다. 이 문제를 피하려고 만들어진 것이 콜백·Promise·async/await다.
...
- `<head>`에 두면 스크립트를 내려받고 실행하는 동안 **HTML 파싱이 멈춰** 화면 표시가 늦어진다
...
| 메모리 저장 | 변수에 **실제 값**이 저장 | 힙(Heap) 주소를 가리키는 **참조값**이 저장 |
...
이 구분이 실제로 문제가 되는 지점은 **복사와 전달**이다. 원시 타입은 값이 복사되지만 객체 타입은 주소가 복사되므로, 복사본을 수정하면 원본도 바뀐다.
...
> 사용자 입력을 그대로 `innerHTML`에 넣으면 XSS(Cross-Site Scripting) 취약점이 된다. 입력된 문자열에 `<script>` 태그가 있으면 그대로 실행될 수 있다. 텍스트만 넣을 때는 반드시 `textContent`를 쓴다.
{: .prompt-danger }
...
타이머 함수는 고유 ID를 반환하며, 이 ID를 보관해 두었다가 필요 없을 때 반드시 해제해야 한다. `setInterval`을 정리하지 않으면 페이지를 벗어난 뒤에도 계속 돌면서 메모리를 낭비한다.
...
|---|---|---|
| 유지 기간 | **영구적** (브라우저를 닫아도 유지) | **임시적** (탭을 닫으면 삭제) |
| 데이터 공유 | 같은 도메인이면 여러 탭에서 공유 | 저장한 탭 안에서만 접근 |
| 활용 예 | 다크 모드 설정, 자동 로그인 토큰 | 일회성 입력 폼, 임시 세션 정보 |
...
- `==` 대신 `===`를 쓴다
- 원시 타입은 값이, 객체 타입은 주소가 복사된다. 이것이 함수 인자 전달과 배열 복사 동작의 근거다
