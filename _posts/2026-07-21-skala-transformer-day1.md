...
**최대 경로 길이 $O(1)$** — 임의의 두 토큰이 **단 한 번의 연산으로 직접 연결**된다. RNN에서 100단어 떨어진 토큰을 잇던 100단계의 곱셈 체인이 사라지므로, 앞서 본 기울기 소실 문제가 구조적으로 제거된다. LSTM이 게이트로 *완화*하던 것을 Transformer는 *제거*한다.
...
여기서 눈여겨볼 비율이 하나 있다. **FFN이 attention보다 파라미터가 2배 많다.** $2 \times 512 \times 2048 = 2.1\text{M}$ vs $4 \times 512^2 = 1.05\text{M}$. Transformer를 "attention 모델"로 부르지만 파라미터의 3분의 2는 FFN에 있고, 이 비율은 현대 LLM에서도 유지된다. MoE(Mixture of Experts)가 하필 FFN을 전문가로 쪼개는 이유가 여기 있다.
...
이 행렬은 **대칭**이다. 즉 "$i$가 $j$를 주목하는 정도"와 "$j$가 $i$를 주목하는 정도"가 항상 같아진다. 하지만 언어의 의존 관계는 비대칭이다. 대명사 `it`은 선행사 `animal`을 강하게 주목해야 하지만, `animal`이 `it`을 같은 강도로 주목할 이유는 없다. **$W^Q \ne W^K$가 비대칭 주목을 가능하게 한다.**
...
**단일 head로 $d_{\text{model}}$ 전체를 쓰는 것과 총 연산량이 같다.** 공짜로 관점을 여러 개 얻는 것이 아니라, 하나의 넓은 관점을 여러 개의 좁은 관점으로 **분할**하는 것이다. 이 트레이드오프를 아는 것이 중요하다 — head 수를 늘리면 각 head의 표현력은 그만큼 줄어든다.
...
차이가 결정적이다. Post-LN에서는 residual 경로가 **LayerNorm을 통과**하므로 항등 경로가 깨진다. 층이 깊어질수록 초기 기울기가 불안정해지고, 원 논문이 **학습률 warmup을 필수로 요구**했던 이유가 여기 있다.

Pre-LN에서는 $x_l$에서 $x_{l+1}$로 가는 **순수한 항등 경로**가 유지된다. Xiong et al.(ICML 2020)이 이를 이론적으로 분석해 Pre-LN이 warmup 없이도 안정적으로 학습됨을 보였고, GPT-2 이후 사실상 표준이 됐다.
...
이 직관에는 정량적 근거도 있다. Dong et al.(ICML 2021)은 **skip connection과 FFN이 없는 순수 self-attention을 쌓으면 출력이 깊이에 대해 이중지수적으로 rank-1에 수렴한다**는 것을 증명했다. 모든 토큰의 표현이 하나의 방향으로 붕괴한다는 뜻이다. 논문 제목이 "Attention is Not All You Need"인 것이 이 결과를 압축한다. **FFN과 residual은 부가 장치가 아니라 붕괴를 막는 필수 요소다.**
...
내적이 **상대 거리 $n-m$에만 의존**하게 되어, 상대 위치가 구조적으로 보장된다. 위 사인파 유도에 등장한 회전행렬을 attention 내부로 옮긴 셈이다. LLaMA, Qwen, Mistral 등 대부분의 오픈 LLM이 채택했다.
...
> 컨텍스트 길이 확장(4K → 128K)이 가능해진 배경에 RoPE의 성질이 있다. 회전 각도의 base를 조정하는 것만으로 위치 보간(position interpolation)이 가능해서, 전체 재학습 없이 파인튜닝만으로 길이를 늘릴 수 있다. 절대 위치 인코딩에서는 불가능했던 일이다.
{: .prompt-tip }
...
- **Transformer의 기여는 attention이 아니라 recurrence 제거**다. 최대 경로 길이 $O(1)$과 순차 연산 $O(1)$이 그 결과이고, 기울기 소실은 완화가 아니라 구조적으로 제거된다
- **$Q \ne K$인 이유는 비대칭 주목**이다. $W^Q=W^K$면 score matrix가 대칭이 되어 언어의 의존 구조를 표현할 수 없다
- **$\sqrt{d_k}$는 내적의 표준편차**다. $\text{Var}(q\cdot k)=d_k$ 이므로 나누면 분산이 1이 되고, softmax가 포화되어 기울기가 죽는 것을 막는다
- **Multi-head는 공짜가 아니다.** $d_k = d_{\text{model}}/h$로 나누므로 총 연산량이 같다. 관점의 수와 각 관점의 표현력이 트레이드오프다
- **파라미터의 2/3는 FFN에 있다.** attention이 아니다
- **FFN과 residual이 없으면 순수 attention 스택은 rank-1로 붕괴한다.** 부가 장치가 아니라 필수 요소다
- **원 논문은 Post-LN, 현대 LLM은 Pre-LN**이다. 항등 경로 보존 여부가 warmup 필요성과 깊이 확장성을 가른다
