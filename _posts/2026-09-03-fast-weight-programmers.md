...
선형화된 자기주의(linearised self-attention)는 과거의 키와 값을 모두 보관하는 대신, 두 벡터의 외적을 누적한 행렬을 매 시점 갱신하는 Fast Weight Programmer로 해석할 수 있다.  
...
표준 Transformer의 자기주의는 길이 $L$인 시퀀스에서 $O(L^2)$의 연산을 요구하고, 과거 키와 값을 저장하는 메모리도 $L$에 따라 선형으로 늘어난다. 컨텍스트 윈도우가 길어질수록 비용이 빠르게 커지므로 실제 모델에서는 처리할 문맥 길이를 제한하게 된다.

선형 트랜스포머 계열은 커널 특징 맵(feature map)을 사용해 연산 순서를 바꾼다. 모든 과거 키와 값을 별도로 유지하지 않고, 값과 변환된 키의 외적을 하나의 고정 크기 행렬에 누적한다. 이렇게 하면 시퀀스 길이에 대해 선형 시간과 상수 크기 상태를 얻을 수 있다.
...
첫째, 선형 트랜스포머의 상태 행렬이 사실상 빠른 가중치(fast weights)이며, 선형 주의는 오래된 Fast Weight Programmer의 특수한 형태라는 점을 보인다.
...
### Softmax를 빼면 자기주의도 같은 형태가 된다

자기주의에서는 입력으로부터 키, 값, 쿼리를 만든다.
...
가 된다. 표준 자기주의 출력은 시간 축에 softmax를 적용한
...
로 계산된다. Eq. 10은 Eq. 2에서 활성화 함수 $\sigma$를 항등 함수로 놓고 $W_a=W_v$, $W_b=W_k$로 대응시킨 형태다. 따라서 softmax가 없는 자기주의는 외적을 누적해 빠른 가중치를 만들고 쿼리로 이를 읽는 FWP다.

이 등가성의 핵심은 단순히 두 수식의 모양이 비슷하다는 데 있지 않다. 선형 트랜스포머의 상태를 압축된 과거 토큰이 아니라 느린 네트워크가 입력마다 다시 쓰는 가중치 행렬로 읽으면, 가산 외에도 삭제나 오차 수정 같은 프로그래밍 명령을 설계할 수 있다.

## 커널 선형화와 상수 크기 상태

Softmax 주의는 커널 형태로 다음과 같이 풀어 쓸 수 있다.
...
기존 선형 주의의 attention normalisation을 유지하려면 누적자
...
Sum normalisation을 사용한 상태에서 attention normalisation을 추가하면 검증 perplexity가 28.1에서 29.2로, 테스트 perplexity가 31.1에서 31.2로 변했다. 비절단 문맥에서는 누적자 $z^{(i)}$가 계속 증가해 수치 불안정의 원인이 됐다. 기존 선형 주의에서 필요했던 정규화가 델타 규칙과 sum normalisation을 결합한 모델에서도 반드시 유리한 것은 아니었다.
...
표준 자기주의는 시퀀스 길이에 대해 $O(L^2)$ 계산을 사용하고, 과거 키와 값을 저장하는 상태가 $O(L)$로 증가한다. 선형 주의와 Delta Network는 매 시점 $d_{value}\times d_{dot}$ 행렬을 갱신하므로 전체 시간은 시퀀스 길이에 선형이고 순차 추론의 상태 크기는 길이와 무관하다.
...
둘째, softmax memory는 합성 검색에서 500개가 넘는 키까지 제안된 선형 주의 변형보다 높은 절대 용량과 성능을 유지했다. DPFP가 $d_{dot}$을 확장해도 유한 차원이라는 사실은 바뀌지 않는다.
...
이 논문이 보여준 중요한 흐름은 효율적 주의를 단순한 softmax 근사 문제로만 보지 않았다는 점이다. 선형 주의를 연상 메모리이자 입력으로 프로그래밍되는 빠른 가중치로 해석하자 특징 맵의 근사 정확도 외에도 메모리 용량, 삭제 규칙, 쓰기 강도, 상태 안정성이라는 설계 축이 드러난다. DPFP는 공간을 넓히고, 델타 규칙은 그 공간을 수정 가능하게 만들며, sum normalisation은 쓰기와 삭제가 같은 척도에서 작동하도록 한다. 세 요소가 각각 다른 실패 원인을 담당한다는 것이 이 논문의 핵심이다.