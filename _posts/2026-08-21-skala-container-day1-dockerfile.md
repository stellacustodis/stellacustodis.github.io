...
- 각 빌드 스테이지는 `FROM`으로 시작한다.
- 첫 `FROM` 앞에 올 수 있는 Dockerfile 명령은 전역 `ARG`뿐이다. parser directive와 주석·빈 줄은 그보다 앞에 올 수 있다.
...
{: .prompt-info }
> `ENV`와의 차이가 자주 헷갈린다. `ARG`는 컨테이너의 런타임 환경변수로 남지 않지만, 사용 방식에 따라 이미지 히스토리·provenance·파일시스템에 흔적이 남을 수 있다. `ENV`는 빌드 시점부터 런타임(`docker run`)까지 유지된다. 둘 다 비밀값을 전달하는 수단으로 쓰면 안 된다.
...
- **빌드 컨텍스트 기준**: `<src>`는 `docker build`에 지정한 빌드 컨텍스트의 루트 기준 상대 경로다. **`../`로 컨텍스트 밖의 파일을 가져올 수 없다.**
...
- `ARG`는 런타임 환경변수로 남지 않고 `ENV`는 런타임까지 이어지지만, 둘 다 이미지에 흔적을 남길 수 있어 비밀값 전달에는 쓰지 않는다.
