...
public List<Document> hydeSearch(String question, int topK) {
    String hypothetical = strict.prompt()
            .system("질문의 의도와 핵심 용어를 유지한 그럴듯한 답변을 3문장으로 쓴다. 검색용 초안이며 확인되지 않은 세부 수치는 만들지 않는다.")
            .user(question)
...
시스템 프롬프트의 `"가상 답변"`이 처음에는 이상해 보였다. 아직 검색하지 않은 내용을 먼저 쓰게 하기 때문이다.

의도를 이해하고 나니 납득이 됐다. 이 텍스트는 사용자에게 가지 않고 **검색 쿼리로만 쓰인다.** 최종 답변처럼 사실을 인용할 필요는 없지만, 내용이 질문 의도에서 벗어나면 엉뚱한 문서를 회수하는 query drift가 생긴다. 따라서 정답 문서와 비슷한 어휘와 문체를 만들되 핵심 의미는 보존해야 한다. `maxTokens(200)`으로 짧게 자른 것도 검색 쿼리로 쓸 만큼만 필요하기 때문이다.
...
교재는 재순위를 두고 "효과 대비 노력이 가장 좋다"고 평가한다. 재순위 뒤 최종 생성에 넘기는 근거가 짧아지면 그 호출의 컨텍스트 토큰은 줄 수 있다. 다만 별도의 LLM 재순위 호출도 입력·출력 토큰을 쓰므로 **전체 토큰·비용은 늘 수 있고 정확도도 측정해야 한다.**
...
- HyDE는 가상 답변으로 검색한다. 사용자에게 직접 보여 주지는 않지만 query drift를 막도록 질문의 핵심 의미는 보존해야 한다.
- 재순위는 최종 컨텍스트를 줄일 수 있지만 별도 호출 비용과 정확도를 함께 측정한다. 모델이 준 인덱스도 검증한다.
