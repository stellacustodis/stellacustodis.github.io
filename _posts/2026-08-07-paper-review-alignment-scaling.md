...
**Kaplan과 왜 달랐나** — 같은 방법론으로 반대 결론이 나온 이유가 흥미롭다. Chinchilla가 직접 지목한 주요 차이 중 하나는 **학습 토큰 수와 학습률 스케줄을 각 실행 길이에 맞추는 방식**이었다. Kaplan의 일부 실험은 고정된 토큰 수와 스케줄 때문에 오래 학습한 설정의 효과를 충분히 모델링하기 어려웠고, Chinchilla는 각 학습 길이에 코사인 스케줄을 맞춰 재실험했다. 다만 이후의 통제된 재현 연구는 마지막 층의 FLOPs 계산, warmup 길이, 규모별 optimizer tuning 등도 지수 차이에 영향을 준다고 보고했다.

수십억 달러 규모의 산업 전략이 학습 스케줄과 연산량 계산을 포함한 실험 설계에 좌우된 셈이다. 경험 법칙의 정확한 지수가 실험 조건에 얼마나 민감한지를 보여주는 사례다.
...
| Chinchilla 2022 | $D/N\approx 20$ — 균형 | Kaplan (실험 설계·FLOPs 산정 차이) |
| LLaMA 2023 | 143 tokens/param — 작게, 오래 | Chinchilla (추론 예산 미고려) |
| Wei 2022 → Schaeffer 2023 | 창발 주장 → 지표 착시 반론 | — |

이 묶음에서 가장 배울 점이 많았던 것은 **Kaplan → Chinchilla → LLaMA의 연쇄 반전**이다. 같은 경험 법칙을 두고 학습 스케줄·연산량 산정·최적화 설정과 최적화 대상(학습 vs 추론)이 바뀌자 산업 전체의 방향이 두 번 뒤집혔다. 논문을 개별적으로 읽었을 때는 각각 타당해 보이는데, 계보로 놓으니 **경험 법칙이 얼마나 취약한 전제 위에 서 있는지**가 드러난다.
