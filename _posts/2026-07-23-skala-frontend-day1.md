...
| Transport | 목적지까지 정확하게 전달 | TCP, UDP | Segment (TCP) / Datagram (UDP) |
...
전송의 신뢰성을 담당하는 계층이며, 여기서 TCP와 UDP가 갈린다.
...
| 용도 | 웹(HTTP), 파일 전송(FTP), 이메일(SMTP) | 실시간 스트리밍, 온라인 게임, DNS 질의 |
...
3. 브라우저 ↔ 웹 서버   : TCP 3-way handshake로 연결 수립
4. 브라우저 → 웹 서버   : HTTP 요청 (GET / HTTP/1.1)
...
실무에서 압도적으로 자주 쓰는 것은 GET과 POST다. 둘의 결정적 차이는 "데이터를 어디에 싣는가"다. GET은 URL 쿼리스트링에, POST는 본문(body)에 싣는다. 그래서 비밀번호를 GET으로 보내면 주소창과 서버 접근 로그에 그대로 남는다.
...
| 4xx | **클라이언트** 잘못 | 400 Bad Request, 401 Unauthorized, 404 Not Found |
| 5xx | **서버** 잘못 | 500 Internal Server Error, 503 Service Unavailable |

4xx와 5xx를 구분하는 습관이 중요하다. 4xx는 요청을 고쳐야 하고, 5xx는 서버 쪽을 봐야 한다. 책임의 소재가 다르다.
...
- **목적지 주소** — 어떤 도메인, 어떤 IP로 연결하는지

패킷을 배달하려면 주소는 읽을 수 있어야 하기 때문이다. 정리하면 HTTPS는 "**어디에 접속했는지는 가려지지 않지만, 무엇을 주고받았는지는 가려진다**"고 이해하면 된다.
...
`<meta charset="UTF-8">`이 빠지면 한글이 깨질 수 있고, viewport 메타 태그가 빠지면 모바일에서 반응형 CSS가 동작하지 않는다. 둘 다 관례가 아니라 기능이다.
...
`<strong>`과 `<em>`은 스크린 리더가 억양을 바꿔 읽어주지만 `<b>`와 `<i>`는 그렇지 않다. 의미상 강조라면 `<strong>`/`<em>`을 쓰는 것이 맞다.
...
크기는 `width`/`height` 속성보다 `style`(CSS)로 지정하는 편이 권장된다. 표현은 CSS의 영역이기 때문이다.
...
`<title>`과 `<meta name="description">`은 검색 결과 목록에 그대로 노출되는 값이라 SEO에 직접 영향을 준다.
...
- 브라우저 주소창의 한 번의 입력 뒤에는 DNS 조회 → TCP 연결 → HTTP 요청·응답 → 렌더링이 있다
- HTTPS는 내용을 가리지만 목적지 주소는 가리지 못한다
