...
중간 추론 단계를 생성하게 만드는 기법이다. 강의가 제시한 장점은 다음과 같다.

- 적은 노력으로 정확도 향상에 기여
- **출력된 중간 단계를 확인해 논리의 어긋난 지점을 점검할 단서를 얻을 수 있다**
- 여러 LLM 간 전환 시에도 강건해질 수 있다

두 번째가 실무적으로 가장 크다. 답만 나오는 경우보다 출력된 과정의 어느 단계에서 논리가 어긋났는지 검토하기 쉽다. 다만 이 텍스트가 모델 내부 계산을 충실히 설명한다는 보장은 없다. 따라서 실제 오답 원인은 중간 주장과 계산을 외부 근거로 다시 검증해 판단해야 한다.
...
핵심 논거는 **기존 CoT 결과를 탐욕적 디코딩(greedy decoding) 한 경로로 고르면 한 번의 잘못된 경로 선택을 되돌리기 어렵다**는 것이다. CoT 자체가 반드시 greedy decoding을 뜻하는 것은 아니다. Self-Consistency는 샘플링으로 여러 추론 경로를 만들고 투표로 이 취약성을 완화한다. 원 논문은 Wang, Wei et al. (Google Research, 2022)이다.
...
- **원 논문의 당시 모델·과제에서는 약 100B 규모부터 CoT 이득이 두드러졌다** — 모든 모델과 과제에 적용되는 보편 임계값은 아니다
