...
```text
AWS EC2 가상화 인스턴스               개발자 맥북
└── Nitro Hypervisor(현행 다수, 일부 구형 Xen)  └── macOS (주 작업 환경)
...
└── 관리 도구 (내장)            └── OS 커널 기능 + 가상화 관리 스택·VSP·WMI/API
```
...
대신 이 실습의 Linux 프로세스 격리 컨테이너에는 조건이 붙는다. **호스트의 Linux 커널을 공유하므로 다른 커널 계열의 OS를 올릴 수 없다.** Windows 컨테이너나 별도 VM 커널을 쓰는 Hyper-V isolation 같은 방식은 이 설명의 범위 밖이다.

그 조건을 받아들이면 앞의 세 문제가 한 번에 정리된다.

| | VM | 컨테이너 |
|---|---|---|
| 이미지 크기 | Guest OS를 포함해 대체로 큼 | 커널을 포함하지 않아 대체로 작음 |
| 기동 속도 | Guest OS 부팅이 필요 | 격리된 프로세스를 시작하므로 대체로 빠름 |
| 이식성 | 호환 하이퍼바이저·가상 하드웨어가 필요 | 호환 커널 계열·CPU 아키텍처·runtime이 필요 |
| 이기종 OS | 가능 | 프로세스 격리에서는 불가능 (Host와 동일 커널 계열, VM 기반 격리는 예외) |
| 집적도 | Guest OS 자원만큼 낮아지는 경향 | 커널 공유로 높아지는 경향. 실제 수는 workload와 제한값에 따라 다름 |
| OS Cost | Guest별 OS 운영·라이선스 비용이 생길 수 있음 | 공유 커널 기준으로 운영하되 이미지·runtime 관리 비용은 남음 |
| 베어메탈 대비 성능 | 하드웨어 가상화·I/O 경로와 workload에 따라 달라짐 | 격리 overhead가 대체로 작지만 고정 비율로 보장되지 않음 |
...
"이 순서로 설치해라"가 아니라 "이런 상태의 이미지가 필요하다"를 적는다. 이 예시에서는 `RUN`과 `COPY`가 파일시스템 레이어를 만들고, `CMD`는 실행 메타데이터를 정한다. 레이어 이야기는 [다음 편](/posts/skala-container-day1-image-registry/)에서 이어진다.
