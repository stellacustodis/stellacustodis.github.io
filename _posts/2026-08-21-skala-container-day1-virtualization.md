...
### 서버 대여 서비스는 컨테이너인가 VM인가

둘 다 있다. 무엇을 빌리는지에 따라 갈린다.
...
"GPU 몇 장"이라고 파는 것은 대개 첫 번째나 세 번째다.

### GPU는 커널 드라이버인데 어떻게 장 단위로 빌려주나

GPU는 PCIe 장치라서 CPU·메모리처럼 잘게 쪼개기 어렵다. 그래서 방식이 갈린다.

**VM에 붙일 때**는 **PCI passthrough**로 물리 카드를 통째로 VM에 넘긴다.
호스트는 그 카드를 포기하고 게스트가 직접 잡는다. 그래서 **장 단위**가 된다.
NVIDIA vGPU 같은 기술로 한 장을 여러 VM에 쪼개 주기도 하지만 별도 라이선스가 필요하다.

**컨테이너에 붙일 때**는 다르다. 컨테이너는 호스트 커널을 공유하므로
**드라이버는 호스트에 한 벌만** 있고, 컨테이너에는 디바이스 파일(`/dev/nvidia*`)과
사용자 공간 라이브러리만 넘긴다. 그 일을 NVIDIA Container Toolkit이 한다.

```bash
docker run --gpus all nvidia/cuda:12.4.0-base nvidia-smi
```

이 글 앞부분의 "커널을 공유한다"가 여기서 실제 제약으로 나타난다.
**컨테이너 안에는 드라이버가 없다.** 그래서 호스트 드라이버 버전이 컨테이너의
CUDA 버전을 감당하지 못하면 이미지를 아무리 잘 만들어도 돌지 않는다.

A100·H100의 MIG(Multi-Instance GPU)는 카드 하나를 하드웨어 수준에서 최대 7조각으로 나눈다.
"1/7장"처럼 파는 서비스가 이걸 쓴다.

### Colab은 VM인가

Colab 런타임은 VM 위에서 동작하고, 노트북 프로세스는 그 안의 **컨테이너**에서 돈다고 알려져 있다.
직접 확인해 볼 수 있다.
...
컨테이너 안이면 PID 1이 `init`이 아니고, cgroup 경로에 컨테이너 식별자가 보인다.
런타임을 초기화하면 상태가 통째로 사라지는 것도 컨테이너의 성질이다 —
변경분은 [2일차 ⑦](/posts/skala-container-day2-kernel/)에서 다루는 upperdir에만 쌓이기 때문이다.

### VM 위에 컨테이너를 올리는 이유

현업의 표준 구성이 맞다. EKS 노드는 EC2 인스턴스이고, 그 위에서 컨테이너가 돈다.

"외부 공격이 없다면 VM이 필요 없다"는 절반만 맞다. **보안이 가장 큰 이유인 것은 맞다.**
컨테이너는 커널을 공유하므로 커널 취약점 하나로 다른 컨테이너나 호스트로 넘어갈 수 있다(container escape).
VM은 하이퍼바이저라는 더 두꺼운 경계를 둔다. 남의 코드를 같은 물리 서버에서 돌리는
멀티테넌시 환경에서는 이 차이가 결정적이다.

그런데 보안 말고도 VM 층이 주는 것이 있다.

- **하드웨어 추상화** — 물리 서버 교체가 위에 영향을 주지 않는다
- **라이브 마이그레이션** — 도는 채로 다른 물리 서버로 옮긴다
- **OS 분리** — 노드마다 커널 버전을 다르게 가져갈 수 있다
- **자원 회계** — 과금과 할당 단위가 명확해진다

그래서 신뢰하는 코드만 돌리는 온프렘 환경에서는 **베어메탈 쿠버네티스**도 실제로 쓴다.
성능은 그쪽이 낫다. 반대로 격리를 더 원하면 [2일차 ⑧](/posts/skala-container-day2-runtime-runc/)에서 언급한
gVisor나 Kata Containers처럼 컨테이너 인터페이스를 유지하면서 격리를 VM 쪽으로 되돌린 런타임을 쓴다.
