...
### 그전에 — Graceful shutdown이 뭔가

**종료 신호를 받고 즉시 죽지 않고, 하던 일을 마무리한 뒤 끝내는 것**이다.
서버 애플리케이션은 보통 이 순서를 밟는다.
...
배포 때 컨테이너를 한 대씩 교체한다고 하면 차이가 이렇게 난다.
...
유예 시간도 알아 두면 좋다. `docker stop`은 기본 **10초**,
쿠버네티스의 `terminationGracePeriodSeconds`는 기본 **30초**를 기다린 뒤 SIGKILL을 보낸다.
**애플리케이션의 종료 타임아웃은 이 값보다 짧게** 잡아야 한다.
